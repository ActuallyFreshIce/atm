...
local XOR_KEY = hexToBytes(sha256(SECRET_KEY))
...
local function loadAccount()
    ...
    if not f then error("account.dat missing") end
    local enc = f.readAll()
    f.close()
    local dec = xorStr(enc, XOR_KEY)
    account = textutils.unserialize(dec)
    if not account then error("Invalid account data") end
    print("[DEBUG] Account loaded: "..account.name.." balance $"..account.debit)
end
